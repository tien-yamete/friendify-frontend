...
export const getMyInfo = async () => {
  return apiFetch(API_ENDPOINTS.USER.MY_PROFILE);
};
...
export const updateProfile = async (profileData) => {
  return apiFetch(API_ENDPOINTS.USER.UPDATE_PROFILE, {
    method: 'PUT',
...
export const uploadAvatar = async (formData) => {
  return apiFetch(API_ENDPOINTS.USER.UPDATE_AVATAR, {
    method: 'PUT',
    body: formData,
  });
};
...
export const search = async (keyword) => {
  return apiFetch(API_ENDPOINTS.USER.SEARCH, {
    method: 'POST',
...
export const getUserProfileById = async (userId, suppress404 = true) => {
  ...
  try {
    ...
    return apiFetch(endpoint, { suppress404 });
  } catch (error) {
